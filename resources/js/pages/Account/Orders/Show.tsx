...
import { FULFILLMENT_STATUS_LABELS, paymentStatusLabel, orderStatusLabel } from '@/components/orders/orderStatus';
...
interface Order {
    ...
    timeline: { status: string; date: string | null }[];
}
                ...
                <section className="rounded-2xl border border-vgs-gray-border bg-vgs-black-surface p-5 sm:p-6">
                    <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                        <div>
                            ...
                            <p className="mt-1 text-xs text-vgs-silver-muted">{order.placed_at || 'Tanggal belum tersedia'}</p>
                        </div>
                ...
                <section className="rounded-2xl border border-vgs-gray-border bg-vgs-black-surface p-5 sm:p-6">
                    ...
                    <div className="mt-5 divide-y divide-vgs-gray-border">
                        {order.items.map((item, index) => (
                            <div key={`${item.sku ?? item.product_name}-${index}`} className="flex gap-3 py-4 first:pt-0 last:pb-0">
                    ...
                    <section className="rounded-2xl border border-vgs-gray-border bg-vgs-black-surface p-5 sm:p-6">
                        ...
                        <div className="mt-4 space-y-3">
                            {order.shipments.map((shipment, index) => (
                                <div key={index} className="rounded-xl border border-vgs-gray-border bg-vgs-black-elevated/50 p-4 text-sm">
                                    ...
                                    {shipment.shipped_at && <p className="mt-1 text-xs text-vgs-silver-muted">Dikirim {shipment.shipped_at}</p>}
                                </div>
                            ))}
                        </div>
                    </section>
