...
export const CartSummary: React.FC<CartSummaryProps> = ({
    ...
    return (
        <div className={`flex flex-col gap-5 p-5 sm:p-6 rounded-2xl bg-vgs-black-surface border border-vgs-gray-border shadow-xl ${className}`}>
            <h3 className="font-display font-bold text-lg text-vgs-silver-bright border-b border-vgs-gray-border pb-3 flex items-center justify-between">
            ...
            {subtotal > 0 && (
                <div className="flex flex-col gap-1.5 p-3 rounded-xl bg-vgs-black-void border border-vgs-gray-border/60">
                    <div className="flex items-center justify-between text-xs">
                        <span className="font-semibold text-vgs-silver-bright flex items-center gap-1.5">
                            ...
                            {isFreeShipping ? 'Gratis Ongkir Turnamen Aktif!' : 'Gratis Ongkir'}
                        </span>
                        <span className="font-mono text-[11px] text-vgs-silver-muted">
                            {isFreeShipping ? 'Tercapai' : `Tambah ${formatRupiah(FREE_SHIPPING_THRESHOLD - subtotal)}`}
                        </span>
                    </div>
                    <div className="w-full h-1.5 rounded-full bg-vgs-gray-border overflow-hidden">
                        <div
                            className={`h-full transition-all duration-300 ${
                                isFreeShipping ? 'bg-vgs-success' : 'bg-vgs-blue-electric'
                            }`}
            ...
            {showPromoInput && subtotal > 0 && (
                <form onSubmit={handleApplyPromo} className="flex flex-col gap-2">
                    <div className="flex items-center gap-2">
                        ...
                        />
                        <Button variant="secondary" size="md" type="submit" className="shrink-0">
                            Pakai
                        </Button>
                    </div>
                    {promoMessage && (
                        <span
                            className={`text-xs ${
                                promoMessage.type === 'success' ? 'text-vgs-success' : 'text-vgs-danger'
                            }`}
                        >
                            {promoMessage.text}
                        </span>
            ...
            <div className="flex flex-col gap-2.5 text-xs sm:text-sm font-medium border-t border-vgs-gray-border/60 pt-4">
                ...
                <div className="flex items-center justify-between text-vgs-silver-mid">
                    ...
                    <span className="font-mono text-vgs-silver-bright font-semibold">
                        {subtotal === 0 ? 'Rp 0' : isFreeShipping ? (
                            <span className="text-vgs-success font-bold uppercase text-xs">GRATIS</span>
                        ) : (
                ...
                {/* Grand Total */}
                <div className="flex items-center justify-between border-t border-vgs-gray-border pt-3 mt-1 text-sm sm:text-base">
            ...
            <div className="flex items-center justify-center gap-2 text-[11px] font-mono text-vgs-silver-muted pt-1">
